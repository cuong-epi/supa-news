...
import { CheckInData } from "./Model";
import moment from 'moment';
import { DatePickerProps, RangePickerProps } from "antd/lib/date-picker";
import UserService from "../../services/UserService";

export default function ListCheckInByDay() {
  ...
  const searchCheckInData = async () => {
    ...
    const res = await service.getTotalUsersCheckInByDateRange(fromDate, toDate);
    setCheckInData(res.data.records ?? [])
    setLoading(false);
  }
  ...
  return (
    <div className="list">
          <Space style={{ marginBottom: 16}}>
              ...
              <Button type="primary" onClick={() => searchCheckInData()}>Search</Button>
          </Space>
         <Table 
            rowKey="date"
            columns={columns} 
